...
int Numbers=0,flagLOOP=0,check=0;
void addKeywords()
{
    keywords.pb("PROGRAM");
    keywords.pb(("VAR"));
...
void FOR(){
    ...
    incr();
    cout<<endl;
    cout<<"LDX #"<<tokens[idx].second<<endl;
    incr();
    if(get()!=9)ERROR();
    incr();
    cout<<"LDT "<<tokens[idx].second<<endl;
    incr();
    if(get()!=10)ERROR();
    incr();
    bool more=0;
    ...
    int k=statements();
      if(check==1){
        ...
        cout<<"TIXR T"<<endl;
        cout<<"JLT LOOP"<<Numbers<<endl<<endl;
    //if(k>1&&more==0)ERROR();
...
int main()
{
   ...
   cout<<tokens[1].second<<" START 1000"<<endl;
    ...
    if(var==1){
         cout<<"USEDVAR10 RESW 1\n";
         cout<<"USEDVAR11 RESW 1\n";
    }
    if(flag1==1 || flag2==1){
        cout<<"INDEV BYTE \"F2\" \n"<<endl;
        cout<<"SCAN RESW 1\n"<<endl;
